...
if systemctl is-active --quiet particleos-module-preload.service &&
        grep -qw sysinit.target <<<"$module_preload_after" &&
        grep -qw nftables.service <<<"$module_preload_before" &&
        grep -qw particleos-module-lockdown.service <<<"$module_preload_before"; then
    pass 'fixed container modules load after sysinit and before firewall lockdown'
else
    fail 'fixed container modules load after sysinit and before firewall lockdown'
fi
