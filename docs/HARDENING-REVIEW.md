...
| TPM2 state encryption | Adapt | One PCR 7-bound LUKS2/Btrfs state partition replaces role-specific root/home/swap layouts; optional PCR11 public-key enrollment is suppressed because the RSA-4096 OBS key is not loadable by common TPMs. |
...
| Post-boot module lockdown | Retain | Required modules load first; subsequent module loading is disabled. |
...
| Role accounts, homed, admin enrollment | Drop | Account and key creation are external provisioning concerns. |
