...
require_fixed 'defcontext=system_u:object_r:user_home_t:s0,rootcontext=system_u:object_r:user_home_dir_t:s0' mkosi.extra/usr/lib/systemd/system/systemd-homed.service.d/40-particleos-selinux.conf 'homed labels unlabeled Btrfs content and its root before PAM enters it'
...
require_fixed 'run0 --pipe /usr/bin/bash -c' tests/firstboot-console-expect.py 'the native account authenticates run0 on its active console'
require_fixed '/usr/sbin/ausearch -m USER_AUTH,USER_ACCT -ts boot' tests/firstboot-console-expect.py 'the console audit requires a kernel authentication record'
require_fixed 'ExecStart=/usr/bin/login' tests/firstboot-console-audit.conf 'the run0 audit creates a real PAM and logind login session'
